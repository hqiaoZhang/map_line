 ...
 * @Last Modified time: 2018-12-07 13:01:50
*/
    ...
    methods: {
        ...
        if (self.validateForm() == true) {
          ...
          fetch('fetchLogin', params, (data) => {
            ...
            if (data.erroCode == 2000) {
              self.$router.push('/weather-flymon/index')
              // 登录成功(状态设置)
              ...
              messagePopup(this, data.erroMsg)
            }
          })
        }
      },
      ...
      validateForm() {
        ...
        if (_.isEmpty(username)) {
          messagePopup(this, '用户名不能为空 !')
          return false
        }
        // 密码验证
        if (_.isEmpty(password)) {
          messagePopup(this, '密码不能为空 !')
          return false
        }
        // 验证码
        if (_.isEmpty(code)) {
          messagePopup(this, '验证码不能为空 !')
          return false
        }
        let realCode = getCookie('code')
        if (_.trim(code.toLowerCase()) != realCode.toLowerCase()) {
          messagePopup(this, '验证码不正确 !')
          return false
        }
        return true
      },
